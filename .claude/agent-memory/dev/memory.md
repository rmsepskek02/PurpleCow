...
## 2026-07-06 — 히트 플래시 강도/지속시간 상향 + 상태 틴트 중 플래시 생략 (`_hitFlashColor` 후속)

**작업 내용:** `Assets/_Project/Scripts/Monster/MonsterBase.cs` 두 곳만 수정 (사용자 명시 승인: "플래시효과에만 집중해서 일단 진행해보자").
1. `_hitFlashColor` `(3,3,3,1)` → `(8,8,8,1)`, `_hitFlashDuration` `0.1f` → `0.4f` (필드 기본값, 45~46번째 줄 부근)
2. `TakeDamage()`에서 `_frozenSecondsRemaining > 0f || _slowSecondsRemaining > 0f || _dotStacks.Count > 0` (얼음/슬로우/화상 지속 여부, `UpdateStatusVisual()`과 동일 판정 기준 재사용)이면 `_flashSecondsRemaining` 갱신을 생략 — 상태 틴트가 흰색 플래시로 끊기지 않도록 처리

**배경:** 오버브라이트 (3,3,3,1) 적용 후에도 실기기에서 히트 플래시가 여전히 안 보인다는 문제. 얼음/화상 지속 틴트는 정상적으로 보여 `SpriteRenderer.color` 곱셈 틴트 메커니즘 자체는 작동함이 확인됨 — 지속시간 0.1초가 너무 짧아 놓치고 있을 가능성이 높아, 우선 훨씬 강하고 길게 만들어 보이는지부터 검증하기로 사용자와 합의. 추가로 상태이상(얼음/화상) 지속 중 노말볼 피격 시 플래시가 뜨지 않았으면 좋겠다는 요청 반영.

**검증:** 수정 전 파일 전체를 Read로 확인 → 두 곳만 Edit 수행 → 수정 후 해당 구간 재확인 + `git diff`로 요청된 두 곳 외 다른 변경이 없음을 확인.

**주요 결정사항:**
- 아이스볼 열 판정 버그 등 다른 이슈는 이번 작업 범위 아님 — `WaveManager.cs`를 포함해 전혀 건드리지 않음
