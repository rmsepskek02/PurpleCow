## 2026-07-06 (실기기 검증 완료된 TODO.md 9번 삭제 + ProjectHistory.md 이관 + ProjectStatus.md 갱신 — 고스트볼 버그)

### 작업 내용
- 사용자가 실제로 테스트해 정상 동작(벽 반사, 바닥 귀환, 몬스터 관통 피해 모두 정상)을 확인한 "고스트볼 미작동 버그" 수정 건을 기존 컨벤션(1·6·7번 사례)에 맞춰 처리: `TODO.md` 9번 항목을 완전히 삭제하고 `ProjectHistory.md`/`ProjectStatus.md`로 이관
- 경로: `Assets/_Project/Docs/TODO.md`, `Assets/_Project/Docs/ProjectHistory.md`, `Assets/_Project/Docs/ProjectStatus.md`
- 작업 전 세 문서 전체를 Read로 확인, 오케스트레이터가 제공한 버그 원인/1차 수정(PR #23)/2차 수정(PR #24)/검증 완료 사실을 그대로 신뢰해 반영

### 결과
- `TODO.md`: `## 9. 고스트볼 미작동 버그` 섹션 전체와 뒤따르는 `---` 구분선 하나를 함께 제거(번호 재부여 없이 남은 2·3·4·8·10번만 유지). "다음 단계" 문단을 "1·6·9번은 ... 이 문서에서 제거되었으며 `ProjectHistory.md`에 이관되었습니다. ... 남은 미구현 항목은 2·3·4·8·10번입니다."로 갱신
- `ProjectHistory.md`: 2026-07-06 날짜 섹션 맨 끝(기존 "선택 Polish 1·6번 실기기 검증 완료" 문단 다음)에 신규 `### 고스트볼 미작동 버그 수정 및 실기기 검증 완료 (_Task/2026-07-06/19-50_ghost-ball-wall-fix)` 섹션 추가. 트리거 전환 시 Wall/Ground가 `OnTriggerEnter2D`로만 처리되는데 이 콜백에 처리 로직이 없던 원인, 1차 수정(PR #23, `HandleWallHit()`/`HandleGroundHit()` 추출 후 트리거 콜백에도 연결)과 그 이후에도 남아있던 문제(트리거는 물리 엔진이 속도를 자동 반사시키지 않음), 2차 수정(PR #24, `ReflectOffTriggerWall()` 신규 추가로 Collider Bounds 가로/세로 비율 기반 반사축 판별 후 직접 속도 반사), 몬스터 관통 피해 유지, 사용자 실제 테스트로 벽 반사·바닥 귀환·관통 피해 모두 정상 확인까지 순서대로 서술
- `ProjectStatus.md`: (1) 7번째 줄 "현재 상태" 요약 문장 끝에 고스트볼 버그 수정 완료 및 `TODO.md` 제거 사실 한 문장 추가. (2) "완료된 작업" 목록 끝에 신규 체크박스 항목 추가(원인/1차·2차 수정/검증 완료를 간결하게 요약). (3) "다음 작업 순서" 2번 항목의 "2·3·4·8·9·10번"을 "2·3·4·8·10번"으로 갱신(9번 제거 반영)
- 세 문서 외 다른 파일은 전혀 수정하지 않음, 다른 TODO 항목(2·3·4·5·8·10번)도 건드리지 않음

### 주요 결정사항
- TODO.md 항목 삭제 시 번호를 재부여하지 않고 기존 번호(2·3·4·8·10)를 그대로 유지 — 1·6번 삭제 때와 동일한 원칙
- ProjectHistory.md 신규 섹션은 기존 2026-07-06 날짜 섹션의 맨 마지막(가장 최근 항목인 "선택 Polish 1·6번 실기기 검증 완료" 다음)에 이어 붙이는 방식으로 추가해 날짜 섹션 내 시간 순서를 유지
- 오케스트레이터가 제공한 PR #23/#24 두 차례 수정 경위(1차 수정 후에도 남아있던 트리거 반사 문제)를 생략하지 않고 두 단계 모두 서술해, 향후 유사한 트리거 기반 반사 버그 재발 시 참고할 수 있도록 함

---

## 2026-07-06 (실기기 검증 완료된 TODO.md 1·6번 삭제 + ProjectHistory.md 이관 + ProjectStatus.md 갱신)

### 작업 내용
- 사용자가 "실기기 검증 완료된 항목은 TODO.md에서 삭제하고 히스토리 문서로 옮기라"고 확인한 지시에 따라, `TODO.md`의 1번(치명타 데미지 텍스트 색상 변경)/6번(몬스터 바닥 도달 시 진동→돌진→소멸 연출) 항목을 완전히 삭제하고 `ProjectHistory.md`/`ProjectStatus.md`에 반영
- 5번(아이스볼 및 몬스터 이동 겹침 방지)은 여전히 "실기기 재검증 대기" 상태라 세 문서 어디에서도 완료로 표시하지 않고 그대로 둠
- 경로: `Assets/_Project/Docs/TODO.md`, `Assets/_Project/Docs/ProjectHistory.md`, `Assets/_Project/Docs/ProjectStatus.md`
- 작업 전 세 문서 전체를 Read로 확인, 과거 7번 항목 삭제 시 사용한 패턴(TODO.md에서 항목+구분선 완전 삭제, ProjectHistory.md에 짧은 완료 문단 추가, ProjectStatus.md 체크박스 갱신)을 참고하되, 실제로는 ProjectHistory.md에 7번 항목의 전용 완료 문단이 존재하지 않음을 grep으로 확인함(지시문이 언급한 정확한 문구는 실제 파일에 없었음) — 그럼에도 지시받은 형식(`### 제목 (실기기 검증 완료)`)을 새로 일관되게 적용

### 결과
- `TODO.md`: `## 1. ...`와 `## 6. ...` 두 섹션을 각각의 내용 및 뒤따르는 `---` 구분선과 함께 완전히 제거(번호 재부여 없이 남은 2·3·4·5·8·9·10번만 유지, 5번은 전혀 건드리지 않음). "다음 단계" 문단을 "1·6번은 ... 이 문서에서 제거되었으며 `ProjectHistory.md`에 이관되었습니다. 5번은 ... 재검증을 기다리고 있습니다. 남은 미구현 항목은 2·3·4·8·9·10번입니다."로 갱신
- `ProjectHistory.md`: 2026-07-06 날짜 섹션의 "선택 Polish 1·5·6번 구현" 문단 바로 다음에 신규 `### 선택 Polish 1·6번 실기기 검증 완료` 문단 추가. 치명타 텍스트 색상(`#FF4B3E`)과 바닥 도달 진동→돌진→소멸 연출(피해 이벤트 시점, 연출 중 상호작용 차단)을 요약하고, 사용자가 Android 실기기에서 최종 검증을 완료했다는 사실을 명시. 5번은 이번 검증에 포함되지 않고 여전히 재검증 대기 상태임을 마지막 문장에 명시
- `ProjectStatus.md`: (1) 7번째 줄 "현재 상태" 요약 문장을 1·6번은 실기기 검증 완료로 `TODO.md`에서 제거, 5번은 회귀 수정·빌드 완료 후 재검증 대기 중임을 구분해서 서술하도록 갱신. (2) "진행 중"/"다음 작업 순서" 섹션을 5번(아이스볼 이동 겹침 방지) 재검증 중심으로 갱신. (3) "## 2026-07-06 선택 Polish 1·5·6번" 섹션 마지막 체크박스 "- [ ] Android 실기기 시각·동작 재검증"을 "- [x] 치명타 텍스트 색상, 바닥 도달 진동→돌진→소멸 연출 Android 실기기 검증 완료" + "- [ ] 아이스볼 이동 겹침 방지 Android 실기기 재검증 대기" 두 줄로 분리
- 세 문서 외 다른 파일은 전혀 수정하지 않음

### 주요 결정사항
- 5번 항목은 어느 문서에서도 완료로 표시하지 않고 "재검증 대기" 상태를 일관되게 유지(사용자의 명시적 제약)
- TODO.md 항목 삭제 시 번호를 재부여하지 않고 기존 번호(2·3·4·5·8·9·10)를 그대로 유지해 다른 문서/task 폴더의 번호 참조와 어긋나지 않도록 함
- ProjectHistory.md에 7번 항목의 전용 완료 문단이 실제로는 존재하지 않았다는 사실을 확인했으나, 사용자가 요청한 형식을 그대로 따르는 것이 우선이라 판단해 동일한 스타일(`### 제목 (실기기 검증 완료)`)로 신규 작성함

---

## 2026-07-06 (TODO.md 7번째 항목 추가 — 몬스터 피격/상태이상 스프라이트 색상 효과)

### 작업 내용
- 오케스트레이터가 사용자와 논의해 확정한 신규 Polish 항목("몬스터 피격/상태이상 스프라이트 색상 효과")을 `Assets/_Project/Docs/TODO.md`의 7번째 항목으로 추가
- 경로: `Assets/_Project/Docs/TODO.md`
- 기존 문서 전체(1~6번 항목, "다음 단계" 섹션)를 먼저 Read로 확인해 구조("현재 상태"/"확정된 목표"/"비고")와 톤을 그대로 따름

### 결과
- 기존 "## 6. ..." 섹션과 "## 다음 단계" 섹션 사이에 "## 7. 몬스터 피격/상태이상 스프라이트 색상 효과" 신규 삽입
  - 현재 상태: `MonsterBase.cs`에 `SpriteRenderer` 참조/피격 색상 로직 부재, 얼음은 `_frozenSecondsRemaining`/`_slowSecondsRemaining` 필드로 지속시간 추적하지만 화상(`ApplyDot()`/`CoDotTick()`)은 코루틴 로컬 변수뿐이라 외부 조회 불가, `Fluffy.prefab`에 몬스터 본체 스프라이트와 `BlockVisual`(발판) 스프라이트가 별도 `SpriteRenderer`로 이미 분리됨, `FireBallSkill.cs`/`IceBallSkill.cs`가 각각 `ApplyDot()`/`ApplyFreeze()`+`ApplySlow()` 호출
  - 확정된 목표 6가지: 피격 시 흰색 hit flash, 아이스볼 지속 중 하늘색 계열, 파이어볼 지속 중 붉은주황 계열, 얼음/화상 동시 적용 시 나중 효과 색이 덮어씀, 지속효과 종료 시 원래 색 복귀, 발판(`BlockVisual`)에는 미적용(몬스터 본체 스프라이트에만 적용)
  - 비고: 화상 상태를 얼음처럼 필드(예: `_burnSecondsRemaining`)로 새로 추적해야 할 것으로 보임, 정확한 색상 RGB/hit flash 지속시간은 구현 착수 시점에 결정
- "다음 단계" 섹션의 "위 6개 항목은..." 문구를 "위 7개 항목은..."으로 갱신
- 코드(.cs)는 전혀 수정하지 않음, 신규 문서 생성이 아니므로 AGENTS.md 인덱스 변경 불필요

### 주요 결정사항
- 오케스트레이터가 이미 코드 확인을 마치고 제공한 내용을 그대로 신뢰해 반영하되, 기존 1~6번 항목의 문장 구조/어투(3단 구성, "~함", "~이다" 혼용 등)를 그대로 유지해 문서 전체의 일관성을 지킴
- 기존 1~6번 항목과 "다음 단계" 섹션 본문은 항목 개수 숫자 변경(6→7) 외에는 전혀 건드리지 않음(외과적 변경 원칙)
