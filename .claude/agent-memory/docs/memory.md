...
### 작업 내용 (추가)
- 캐릭터 프리팹 구성 + 무기 조준 회전 애니메이션 task research.md/plan.md 재수정 (1차 구현 완료·커밋 이후 설계 재확정 반영)
- 경로: `Assets/_Project/Docs/_Task/2026-07-05/04-03_character-weapon-aim/research.md`, `.../plan.md`
- 배경: 오케스트레이터가 이미 완료된 1차 구현(weapon meta rect 수정, `CharacterAimController.cs`, `CharacterSetupEditor.cs`, `SceneSetupEditor.cs` Step11) 이후 사용자와 추가 논의하고, `targetUI` 레퍼런스 스크린샷 중 캐릭터가 보이는 6장을 Python(PIL)으로 캐릭터 부분만 크롭(세로 62~90%)해 2배 확대 재분석한 결과를 전달받아 문서만 반영(코드 구현은 이번 범위 아님, dev 에이전트에게 별도 위임 예정)

### 결과
- research.md: 기존 내용 끝에 "추가 조사 — 레퍼런스 스크린샷 재크롭 분석" 섹션 신규 추가. 게임플레이 장면 4장(스킬 선택 UI 4장은 캐릭터가 흐려 분석 불가) 공통 확인 사항(조준 각도 0~90도 범위, 방울 왼쪽·무기 오른쪽 자세, 무기 손잡이는 어깨 근처 고정·갈고리 끝만 호를 그림, 왼쪽 조준 예시는 10장 중 전무, 머리 회전은 0~10도로 제한된다는 사용자 실플레이 경험 근거) 기록
- plan.md: 무기 회전 관련 핵심 설계 4가지를 전면 수정. (1) 18도 각도 오프셋 보정 삭제(3단계), (2) 회전축을 무기 자신의 pivot에서 캐릭터 쪽 고정점인 `WeaponPivot`(빈 부모 오브젝트)으로 이전(2단계, 4단계 계층 구조에 `WeaponPivot` 추가), (3) 조준 각도 부호에 따라 `Character` 루트 `localScale.x`를 ±1로 전환하는 좌우 반전 추가(5단계, 부모 반전 시 자식 회전 부호도 보정 필요하다는 주의사항 포함), (4) `Head`가 조준 각도를 `Mathf.Clamp(-10,10)`로 제한한 값만큼 자기 자신의 `localRotation`으로 tilt하는 머리 회전 추가(4~5단계). 1단계(meta rect 수정)와 6단계(LaunchPoint 정합, WallFitter 실기기 테스트로 동적 추적 스크립트 불필요 확정)는 변경 없이 유지. 문서 서두에 "개정 이력" 문단을 추가해 1차 구현 이후 재확정임을 명시. 주의사항에 1차 구현분(`CharacterAimController.cs`, `CharacterSetupEditor.cs`)을 dev 에이전트가 재작성해야 한다는 점과, 좌우 반전 자세가 레퍼런스 이미지로 검증되지 않았다는 점을 재차 명시

### 주요 결정사항
- 이번 문서 수정 자체에서는 코드를 전혀 건드리지 않음(요청에 명시된 대로 Read/Edit/Write만 사용, Bash 미사용). 코드 재작성은 다음 단계에서 dev 에이전트가 담당
- Weapon 스프라이트의 커스텀 pivot(0.18, 0.29) 유지 여부는 이번에 강제로 결정하지 않고, 기본값(0.5,0.5) 복귀도 허용 범위로 남겨 Scene 뷰 시각 검증으로 최종 확정하도록 서술(과신 금지 원칙 유지)
