...
- **(구현 완료, E)** 가로 2칸(`TwoByOne`, StoneBug)/세로 2칸(`OneByTwo`, ForestDeer) 몬스터가 앵커로 쓰인 한 칸의 중심 좌표로만 배치되어 스프라이트가 인접 칸에 걸쳐 보이거나 그리드 밖으로 튀어나가던 위치 버그가 수정되었다. `PlaceMonster(MonsterData data, Vector3 worldPosition)`가 (col, row) 대신 **최종 월드 좌표(Vector3)**를 직접 받도록 시그니처가 변경되었고, 호출부(`TryDispenseRoster()`/`SpawnRosterAcrossFullGrid()`)가 `BlockSize`에 따라 위치를 계산해서 넘긴다 — `OneByOne`은 그대로 `GridToWorldPosition(col, row)`, `TwoByOne`은 `(GridToWorldPosition(col, row) + GridToWorldPosition(col+1, row)) / 2f`(두 칸의 평균), `OneByTwo`는 그 몬스터가 차지하는 두 행의 `GridToWorldPosition` 결과 평균이다(`TryDispenseRoster()`에서는 `topRow`/`belowRow` 두 행, `SpawnRosterAcrossFullGrid()`에서는 `row`/`row+1` 두 행을 평균 — 두 메서드에서 평균 낼 대상 행이 다름에 유의). 부수적으로 `IsCellFree(col, row)`의 점유 판정 여유 반경도 `checkRadius = _gridCellSize * 0.55f`로 확대되어(기존 `_gridCellSize / 2` 미만·엄격한 `<` 부등호였던 경계 케이스 오판 위험 해소), 2칸 몬스터가 정확히 두 칸의 "중간 지점"에 위치해도 올바르게 "점유됨"으로 판정된다.
...
- **(구현 완료, E)** `PlaceMonster(MonsterData data, Vector3 worldPosition)`로 시그니처가 변경되어 2칸 몬스터의 배치 위치가 더 이상 앵커 칸 중심으로 고정되지 않는다. `IsCellFree()`의 여유 반경도 `checkRadius = _gridCellSize * 0.55f`로 확대되어 경계값 오판 위험이 해소되었다(상세는 위 본문 참고).
...
- **(구현 완료)** 위 블록 크기 매핑은 `MonsterData._blockSize`(`BlockSize` enum: `OneByOne`/`TwoByOne`/`OneByTwo`) 필드로, 점유 체크 로직은 `WaveManager.IsCellFree()`로 실제 코드에 반영되어 있다. `MonsterBase.ApplyBlockSize()`가 `ColliderSizeMap`/`HpBarWidthMap`(모두 `BlockSize`를 키로 하는 `Dictionary`)을 참조해 콜라이더 크기(`OneByOne` 0.96x0.96, `TwoByOne` 1.92x0.96, `OneByTwo` 0.96x1.92)와 HP바 폭(`OneByOne`/`OneByTwo` 1, `TwoByOne` 2)을 스폰/데이터 주입 시점(`OnSpawn()`/`ApplyData()`)마다 자동으로 적용한다.
...
A~G 전체가 위와 같이 이미 `WaveManager.cs`에 구현되어 커밋된 상태임이 이번 확인으로 재검증되었다. 아래 E는 `PlaceMonster()`/`IsCellFree()`에 반영된 좌표 보정 내용이다(구현 완료, 상세 배경은 2장 참고).

- **(구현 완료, E)** 과거에는 `PlaceMonster(MonsterData data, int col, int row)`가 `monster.transform.position = GridToWorldPosition(col, row)`(한 칸짜리 셀의 중심)를 그대로 배치 위치로 사용해, 가로 2칸(`TwoByOne`)/세로 2칸(`OneByTwo`) 몬스터가 실제로 차지하는 두 칸의 중심이 아니라 항상 앵커로 쓰인 한 칸의 중심에 배치되는 버그가 있었다. 이는 `PlaceMonster(MonsterData data, Vector3 worldPosition)`로 시그니처를 변경해 (col, row) 대신 최종 월드 좌표(`Vector3`)를 직접 받도록 수정되었고, 호출부(`TryDispenseRoster()`/`SpawnRosterAcrossFullGrid()`)가 `BlockSize`에 따라 위치를 계산해 넘긴다 — `OneByOne`은 `GridToWorldPosition(col, row)` 그대로, `TwoByOne`은 `(GridToWorldPosition(col, row) + GridToWorldPosition(col+1, row)) / 2f`, `OneByTwo`는 그 몬스터가 차지하는 두 행의 `GridToWorldPosition` 평균이다(`TryDispenseRoster()`에서는 `topRow`/`belowRow`, `SpawnRosterAcrossFullGrid()`에서는 `row`/`row+1` — 두 메서드에서 평균 낼 행이 다름에 유의). 부수적으로 `IsCellFree(col, row)`의 점유 판정 여유 반경도 `checkRadius = _gridCellSize * 0.55f`로 늘어나, 2칸 몬스터가 정확히 두 칸의 중간 지점에 위치했을 때 거리가 정확히 반 칸이 되어 "비어있음"으로 오판되던 경계 케이스가 해소되었다.
