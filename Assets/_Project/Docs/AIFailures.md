...
- **실패 내용**: 최초 구현은 아이스볼 적중 순간 존재한 후방 몬스터에게만 Freeze/Slow를 적용했고, 스폰 가능 여부는 기존 중심점 거리 검사에 맡겼다. 사용자 플레이 테스트에서 빙결 이후 새로 스폰된 몬스터가 계속 내려와 겹쳤으며, 일부는 생성 순간부터 Collider가 겹쳤다.
- **원인**: 지속시간이 있는 상태이상을 한 시점의 대상 목록만으로 해결해 이후 합류하는 오브젝트를 고려하지 않았다. 또한 `IsCellFree()`가 실제 Collider Bounds 대신 `_gridCellSize * 0.55` 중심점 반경을 사용해, 중심 거리는 반경 밖이지만 Collider는 여전히 겹치는 구간을 빈 셀로 오판했다.
- **재발 방지**: 지속 효과로 공간 점유가 달라지는 기능은 적중 순간뿐 아니라 효과가 유지되는 전체 시간축에서 신규 생성·이동을 검토한다. 스폰·충돌 가능 여부는 시각적 중심 거리 근사 대신 실제 Collider Bounds의 교집합을 기준으로 검증한다. 이번 수정에서는 이동 직전 아래쪽 빙결 몬스터를 동적으로 조회하고, 스폰 셀과 활성 몬스터 Collider의 x·y 양수 교집합을 검사하도록 변경했다.
