...
- **(구현 완료)** 아래 배치 방식은 `MonsterOverhaulSetupEditor.cs`(프리팹 구조 변경)와 `MonsterBase.ApplyBlockSize()`(런타임 크기 적용)를 통해 실제 코드에 반영되어 있다.

### 배치 방식 (앵커/부모 구조 변경)

- 기존: 몬스터 프리팹 최상위에 `World Space Canvas`를 자식으로 두고, 몬스터 머리 위쪽으로 오프셋된 위치에 배치.
- 신규: `MonsterOverhaulSetupEditor.SetupPrefabBlockVisuals()`가 프리팹에 순수 시각용 자식 `BlockVisual`(블록 스프라이트, 콜라이더 없음)을 추가하고, 기존 `HpBarCanvas`를 이 `BlockVisual`의 자식으로 재부모화한 뒤 블록 종류별 오프셋(`HpBarYOffset`, 예: 1칸/가로2칸은 -0.33, 세로 2칸 ForestDeer는 -0.81)만큼 아래로 내려 블록 앞면 하단에 배치한다. HP바의 `RectTransform` 가로 크기(`sizeDelta.x`)는 런타임에 `MonsterBase.ApplyBlockSize()`가 `MonsterData.BlockSize`에 따라 1칸 블록은 1, 가로 2칸(`TwoByOne`, StoneBug)은 2로 설정한다(세로 2칸 `OneByTwo`는 폭 1 유지).
- Canvas 설정은 기존과 동일하게 유지한다: Render Mode = World Space, Sorting Layer = UI.

### 재사용된 부분

- `MonsterHpBar` (MonoBehaviour)와 `MonsterBase.OnHpChanged(float current, float max)` 이벤트 구독 구조는 배치 방식만 바뀌었을 뿐 그대로 재사용되었다.
- `MonsterBase`의 `public event Action<float, float> OnHpChanged` 발행 시점(TakeDamage/Die/OnSpawn/ApplyData)도 변경 없이 그대로 사용한다.
- HP 0이 되면 HP바 오브젝트는 풀 반납 시 자동 비활성화되는 동작도 유지된다.
- 변경된 것은 HP바의 **부모 오브젝트(월드 캔버스의 부착 위치)와 앵커/크기 설정**뿐이다.
