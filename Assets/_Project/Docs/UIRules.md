...
Canvas_HUD    (Sort Order: 10)  ← 항상 표시
  ...
  │    └─ HUD_Dynamic Canvas    ← HP바, 진행바, 텍스트
  ├─ TopBar   (스테이지명, %, 아이콘*)
  ├─ CharacterXP (캐릭터 경험치바 + 레벨 텍스트 — TopBar 바로 아래 상단 배치**)
  ├─ TopButtons (▶ 재생, ⏸ 일시정지)
...
>
> `TopBar`의 % 진행바는 **스테이지(게임) 전체 누적 처치율**을 의미하며, 현재 웨이브 내 처치율이 아니다. 몬스터가 처치될 때마다 갱신되고, 웨이브가 바뀌어도 리셋되지 않으며, 스테이지(20웨이브) 전체를 클리어하면 100%에 도달한다.
>
> \*\* 이 문서는 당초 상단에 `TopBar`와 별개로 "웨이브 진행바 + 웨이브 번호 배지"(WaveBar)가 있다고 서술했으나, 이는 잘못된 추측이었다. 원본 게임 실제 플레이 확인 결과 이 요소는 실제로는 캐릭터 경험치(XP)바이며, 경험치가 다 차면 레벨업 후 삼택지 스킬 선택 UI가 열리는 바로 그 시스템(`CharacterXpBar.cs`/`CharacterManager`)과 동일하다. 별도의 "웨이브 진행" 개념은 존재하지 않으며, 기존에 하단 배치로 계획했던 `CharacterXP`를 상단(`TopBar` 바로 아래)으로 재배치하는 것으로 통합한다. 이 정정의 배경은 `_Task/2026-07-04/20-04_gameplay-hud-and-skill-selection-ui/research.md`의 추가 정정 기록을 참고.
...
- HUD: `CharacterXpBar` (MonoBehaviour, Slider + TMP_Text 레벨 표시), `CharacterXP` 오브젝트에 부착(1번 섹션 참고 — 상단 배치)
