...
- 테스트 기간에는 좌측 상단에 `S`, `F` 버튼을 가로로 배치합니다.
- `S`는 가장 바깥쪽에서 우측 일시정지 버튼과 대칭 위치에 배치하고, `F`는 그 오른쪽에 둡니다.
- 두 버튼은 일시정지 버튼과 동일한 크기와 시각 스타일을 사용합니다.
- `S`는 성공 결과, `F`는 실패 결과를 기존 `GameManager.EndGame` 흐름으로 호출합니다.
- 두 버튼은 결과 팝업 검증을 위한 테스트 전용 UI이며 원본 게임 UI 요소가 아닙니다.

### 5-4. 캐릭터 HP바

- 캐릭터 HP바는 캐릭터 바로 아래에 붙는 작은 World Space UI입니다.
- 화면 하단 전체 폭을 사용하는 Slider로 만들지 않습니다.
- `104×24` 크기를 기준으로 초록색 Fill, 어두운 배경, 밝은 외곽선을 사용합니다.
- 중앙에 `현재 HP/최대 HP` 형식으로 표시합니다.
- HP 텍스트는 14pt 진한 갈색과 크림색 외곽선을 사용해 초록 Fill과 어두운 빈 영역 모두에서 읽히게 합니다.
...
- 볼 발사, 귀환, 궤적 프리뷰는 `LaunchPoint`를 동일하게 사용합니다.
- `WallFitter._nativeLaunchPointY`는 `-6.7`, 하단 벽 중심은 `-7.5`를 사용합니다.
- 발사 지점은 하단 벽 상단과 공 반지름을 고려해 최소 `0.35` 월드 단위의 간격을 강제로 보장합니다.
- 캐릭터는 `LaunchPoint`의 자식으로 유지하되 로컬 Y `-0.4` 오프셋을 적용해 배경 격자 최하단보다 아래에 표시합니다.
...
- 캐릭터 레벨은 Lv.1에서 시작하고 최대 Lv.19입니다.
- Lv.2 필요 경험치는 50이며 이후 레벨마다 요구량이 18씩 증가합니다.
- 최대 Lv.19는 액티브 4개와 패시브 2개를 각각 Lv.3까지 성장시키는 총 18회의 선택에 대응합니다.
