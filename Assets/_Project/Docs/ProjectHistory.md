...
### 고스트볼 미작동 버그 수정 및 실기기 검증 완료 (`_Task/2026-07-06/19-50_ghost-ball-wall-fix`)

`TODO.md` 9번으로 기록되어 있던 고스트볼 미작동 버그의 원인을 확정하고 두 차례에 걸쳐 수정하였다.

- **원인**: 고스트 모드에서 `Ball.SetGhostMode(true)`가 볼 자신의 Collider2D를 `isTrigger = true`로 전환하는데, Unity 2D 물리 규칙상 두 Collider 중 하나라도 트리거면 그 쌍의 상호작용은 `OnCollisionEnter2D`가 아닌 `OnTriggerEnter2D`로만 발생한다. 그런데 `Ball.cs`의 `OnTriggerEnter2D`는 Monster 태그만 처리하고 Wall/Ground 처리가 전혀 없어, 고스트볼이 벽/바닥과 접촉해도 아무 반응 없이 그대로 뚫고 나가버렸다.
- **1차 수정(PR #23)**: `Ball.cs`의 기존 Wall/Ground 처리 로직을 `HandleWallHit()`/`HandleGroundHit()`로 추출해 `OnCollisionEnter2D`뿐 아니라 `OnTriggerEnter2D`에서도 호출되도록 연결하였다.
- **잔존 문제 및 2차 수정(PR #24)**: 1차 수정 이후에도 문제가 남아 있었다. 트리거 콜라이더는 Unity 물리 엔진이 속도를 자동으로 반사시켜주지 않는다는 점을 놓쳐, 이벤트 자체는 감지되지만 볼이 방향을 바꾸지 않고 그대로 벽을 통과해버렸다. `ReflectOffTriggerWall()`을 신규 추가해 Collider Bounds의 가로/세로 비율(`Wall_Left`/`Wall_Right`는 세로로 긴 벽, `Wall_Top`은 가로로 넓은 벽)로 반사축을 판별한 뒤, 트리거 경로에서만 볼의 속도를 직접 반사시키도록 수정하였다.
- **유지된 동작**: 고스트볼이 몬스터를 관통하며 피해만 입히고 충돌 반응은 없는 기존 피어싱 동작은 그대로 유지되었다.
- **검증**: 사용자가 실제로 테스트해 벽 반사, 바닥 귀환, 몬스터 관통 피해가 모두 정상 동작함을 확인하였다. 상세 내용은 `Assets/_Project/Docs/_Task/2026-07-06/19-50_ghost-ball-wall-fix/`에 기록되어 있다.

### 레이저볼/DoT 대미지 텍스트 미표시 수정 및 실기기 검증 완료 (`_Task/2026-07-06/20-14_damage-text-event-fix`)

`TODO.md` 8번(레이저볼 가로 행)과 10번(지속 대미지 DoT)으로 기록되어 있던 대미지 텍스트 미표시 버그를 함께 수정하였다.

- **원인**: 레이저볼 가로 행 부가 피해(`LaserBallSkill.OnBallHit()`)와 DoT 틱 피해(`MonsterBase.UpdateDot()`) 둘 다 `Ball.CalculateDamage()`를 거치지 않고 `MonsterBase.TakeDamage()`를 직접 호출해, `DamageTextManager`가 유일하게 구독하는 `Ball.OnHitMonster` 이벤트가 발행되지 않아 대미지 텍스트가 표시되지 않았다.
- **1차 수정(PR #25)**: 두 호출부에서 각각 `Ball.OnHitMonster?.Invoke(...)`를 직접 호출하도록 추가하였다.
- **빌드 오류 및 2차 수정(PR #26)**: 1차 수정 이후 C# 이벤트는 선언 클래스 내부에서만 Invoke가 가능해 `CS0070` 컴파일 오류가 발생하였다. `Ball.cs`에 `public static void RaiseHitMonster(MonsterBase, float, bool)` 공개 정적 메서드를 신설해 이벤트 발행을 감싸고, 두 호출부가 이 메서드를 사용하도록 수정해 해결하였다.
- **비고**: `isCritical`은 두 경로 모두 항상 `false`로 고정되어 일반 피해와 시각적으로 구분되지 않는다. 기존 볼 직접 충돌 경로(`Ball.CalculateDamage()`)는 변경 없이 그대로 유지되었다.
- **검증**: 사용자가 실제로 테스트해 정상 동작을 확인하였다. 상세 내용은 `Assets/_Project/Docs/_Task/2026-07-06/20-14_damage-text-event-fix/`에 기록되어 있다.

### 몬스터 이동 겹침/관통 버그 수정 및 실기기 검증 완료 (`_Task/2026-07-06/21-25_monster-overlap-freeze-fix`)

`TODO.md` 5번(아이스볼 및 몬스터 이동 겹침 방지)은 구현 완료 후 Android 실기기 재검증 대기 상태였으나, 사용자가 실기기 테스트에서 1×1 몬스터끼리, 1×1과 2칸짜리 몬스터가 완전히 겹치거나 얼어서 멈춘 몬스터를 다른 몬스터가 레인 중상단에서도 관통하는 추가 버그를 발견하였다.

- **1차 조사(기각됨)**: 처음에는 "스프라이트가 Collider보다 세로로 조금 크다"는 가설과 "빙결 중 `BeginBottomAttack` 시작(하단 경계 한정)" 가설을 제시했으나, 사용자가 실제 목격한 현상의 규모(거의 완전한 겹침)와 발생 위치(레인 중상단 포함)에 맞지 않는다고 반박하여 기각되었다.
- **재조사로 확정한 원인**: `ProjectSettings/Physics2DSettings.asset`의 `m_AutoSyncTransforms: 0`(비활성화)과, `MonsterBase.Update()`가 `FixedUpdate`가 아닌 `Update()`에서 `transform.position`을 직접 이동시키는 방식이 결합되어, `WaveManager.GetSafeDownwardDistance()`가 참조하는 `Collider2D.bounds`가 실제 위치보다 최대 한 물리 스텝(0.02초)만큼 지연되었다. 프레임레이트 상한이 없어 50Hz(Fixed Timestep)보다 높은 환경에서는 물리 스텝 하나 안에 `Update()`가 여러 번 실행되며 오차가 누적되어, 몬스터 종류·빙결 여부와 무관하게 겹침·관통이 발생했다.
- **수정(PR #28)**: `WaveManager.Awake()`에서 `Physics2D.autoSyncTransforms = true`로 전역 설정해 물리 질의 시점에 Transform 변경이 항상 즉시 반영되도록 하였다(1개 파일, 1줄 변경). 하단 경계 근처 빙결 중 `BeginBottomAttack` 이슈는 사용자가 실제로 겪지 않았다고 확인되어 이번 수정 범위에서 제외하였다.
- **검증**: 사용자가 실제 플레이 테스트로 얼음볼로 정지시킨 몬스터를 뒤따르는 몬스터가 관통하지 않고 정지하는 것, 몬스터 겹침 현상이 보이지 않는 것을 확인하였다. 상세 내용(1차 조사 기각 경위 포함)은 `Assets/_Project/Docs/_Task/2026-07-06/21-25_monster-overlap-freeze-fix/`에 기록되어 있다.
