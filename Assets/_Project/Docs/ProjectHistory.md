...
- 모든 TMP 텍스트를 `Maplestory Bold SDF.asset`으로 통일하고, 상단 스테이지 진행률/캐릭터 XP/레벨, 캐릭터 하단 HP `현재/최대`, 우측 하단 버서크·분신 버튼을 구성하였다.
...
- Unity 플레이 모드에서 전용 볼 외형, 확률 효과, XP 진행과 최대 레벨 도달을 확인하는 최종 검증은 사용자 로컬 테스트 단계로 남아 있다.
