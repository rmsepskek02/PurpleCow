...
**단계**: 실제 플레이 테스트 진행 중 — 볼-볼 물리 충돌 방지 수정까지 사용자 실기기/로컬 검증 완료(단, 아직 main 미병합 브랜치 상태), 볼 궤적 프리뷰 고리(Ring) 점선화+회전 효과는 구현 완료했으나 최종 시각 확인은 사용자 로컬 Unity 테스트 대기 중
...
**다음 작업 순서**
1. 볼-볼 물리 충돌 방지 브랜치(`claude/project-review-bugs-qq65d1`)를 main에 병합
