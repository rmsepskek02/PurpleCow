...
- 현재 `OnCollisionEnter2D`의 `"Wall"` 분기(`_remainingBounces` 소진 시 `ReturnToPool()`, Ball.cs:83-91)와 `"Ground"` 분기(무조건 `ReturnToPool()`, Ball.cs:92-95) 중, 귀환 트리거는 오직 `"Ground"` 충돌로만 한정한다(확정, 사용자 재확인). `"Wall"`(좌우/상단 벽) 충돌은 로스터 소속 볼에 대해서는 반사 횟수(`_remainingBounces`) 개념 자체를 적용하지 않으며, 몇 번을 튕기든 순수 반사만 하고 절대 귀환을 트리거하지 않는다. 로스터 밖의 볼(서브볼 등)만 기존처럼 `_remainingBounces` 소진 시 `ReturnToPool()`된다.
- 이미 귀환 중(`"Ground"` 충돌 이후 `LaunchPoint`로 이동 중, `_isReturning == true`)인 로스터 볼이 도중에 벽에 부딪히는 경우에는, 반사 카운트와 무관하게 `ReturnToLaunchPoint()`를 다시 호출해 방향을 `LaunchPoint` 쪽으로 재조준한다(귀환 이동이 벽 충돌로 끊기지 않도록).
