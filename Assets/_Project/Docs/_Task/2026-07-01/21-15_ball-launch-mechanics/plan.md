...
- 현재 `OnCollisionEnter2D`의 `"Wall"` 분기(`_remainingBounces` 소진 시 `ReturnToPool()`, Ball.cs:83-91)와 `"Ground"` 분기(무조건 `ReturnToPool()`, Ball.cs:92-95) 중, 귀환 트리거는 `"Ground"` 충돌로만 한정한다(확정). `"Wall"`(좌우/상단 벽) 충돌은 귀환을 트리거하지 않으며 기존처럼 순수 반사(반사 카운트 차감)만 유지한다(확정).
