...
| `Assets/_Project/Scripts/Wave/WaveManager.cs` | `BallLauncher.OnAllBallsReturned`를 구독해 `HandleAllBallsReturned()`(WaveManager.cs:82) → `MoveAllMonstersDown()`(WaveManager.cs:88)으로 몬스터를 한 칸 내리고 게임오버 체크 | 재발사 로직이 생기면 "모든 볼이 반환된 시점"의 의미(사이클 종료 시점 등)가 바뀔 수 있어 영향 범위에 포함. 사용자 확인 사항(몬스터는 시간 기반으로 서서히 내려와야 함)과도 다른 방식이며, 시간 기반 연속 하강 로직 자체를 이번 task 범위에 포함해 함께 구현하기로 확정됨(`OnAllBallsReturned` 구독 및 `MoveAllMonstersDown()` 완전 제거 대상) |
...
| `Assets/_Project/Scripts/Data/MonsterData.cs` | `_moveSpeed`/`MoveSpeed` 필드 보유 | 현재 어디서도 참조되지 않는 죽은 데이터. 시간 기반 연속 하강 구현이 이번 task 범위에 포함되므로, `MonsterBase`의 매 프레임 하강 로직에서 이 값을 실제로 사용하게 됨 |
...
### 몬스터 하강 결합 문제 (사용자 확인 사항 반영, 이번 task 범위 포함 확정)

- 사용자 확인: 몬스터 하강은 볼 발사/귀환 사이클과 무관하게 시간 기반으로 서서히(continuous) 진행되는 별개의 로직이며, 현재 코드처럼 "모든 볼이 반환되는 시점마다 한 칸씩" 내려오는 방식이 아니다.
- 이 "서서히 내려오는" 신규 로직은 사용자가 최종 확인해준 대로 **이번 task 범위에 포함하여 시간 기반 연속 하강 로직까지 함께 구현한다**. 이번 재설계로 볼이 로스터 전체(N개)가 동시에 영구 순환하는 구조가 되면, 발사→귀환이 끊임없이 겹쳐서 일어나 활성 볼 카운트(`_activeBallCount`)가 사실상 0에 도달하는 일이 거의 없어지고, 그 결과 현재 `BallLauncher.OnAllBallsReturned` 이벤트 자체가 (의도한 대로는) 더 이상 발생하지 않게 된다는 점이 이번 결정의 배경이다.
...
- 이 부작용에 대한 처리 방식은 사용자 확인을 거쳐 확정되었다: `OnAllBallsReturned` → `MoveAllMonstersDown` 연결고리를 완전히 제거하고, 몬스터가 매 프레임 스스로 시간 기반으로 연속 하강하도록 이번 task에서 직접 구현한다. 구체적인 구현 설계는 **plan.md에서 다룬다.**
...
- **몬스터가 시간 기반으로 서서히(continuous) 내려오는 실제 로직 구현은 이번 task 범위에 포함하기로 최종 확정됨.** 로스터 구조(위 4번) 도입의 부작용으로 `BallLauncher.OnAllBallsReturned`가 사실상 발생하지 않게 되어, 이 이벤트에 의존하는 `WaveManager`의 몬스터 하강 트리거와 `HUDPanel`의 조준 인디케이터 on/off 트리거가 함께 멈추게 되는데, 이를 임시로만 처리하지 않고 `WaveManager`의 이벤트 결합을 완전히 제거한 뒤 몬스터가 매 프레임 스스로 시간 기반으로 연속 하강하는 로직을 이번 task에서 직접 구현한다. 구체적인 기술 설계는 plan.md에서 다룬다.
