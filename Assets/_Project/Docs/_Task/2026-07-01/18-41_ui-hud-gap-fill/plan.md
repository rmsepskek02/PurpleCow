...
- Editor 스크립트 자동화: `Assets/_Project/Scripts/Editor/UISetupEditor.cs`의 `Step2_SetupHUDCanvas()`에서 이미 `CharacterHP` 오브젝트와 `Slider`, `CharacterHpBar` 컴포넌트를 생성하고 있으므로, 같은 위치에 `EnsureChildObject(charHpObj.transform, "HpText")`로 TMP_Text 자식 오브젝트를 추가하고 `SerializedObject`로 `_hpText` 필드에 연결하는 코드를 추가한다. 기존 `Step9_SetupHUDPanelContent()` 등에서 사용 중인 "`EnsureChildObject` → `TextMeshProUGUI` 추가 → `SerializedObject.FindProperty("_필드명").objectReferenceValue` 연결" 패턴을 그대로 따른다. `[MenuItem("PurpleCow/Setup/UI Setup")]` 메뉴 실행 시 자동으로 생성/연결되므로 수동 Inspector 작업은 불필요하다.
...
- Editor 스크립트 자동화: `UISetupEditor.cs`의 `Step6_CreateSkillCardPrefab()`에 데미지 텍스트 생성/연결 로직을 추가한다. 현재 이 메서드는 `Icon`/`NameText`/`DescText`/`TypeText`/`SelectButton` 자식 오브젝트를 `new GameObject(...)` + `AddComponent<TextMeshProUGUI>()`로 생성한 뒤 `PrefabUtility.SaveAsPrefabAsset()`으로 저장하고, 이어지는 "Always connect internal refs" 블록(`PrefabUtility.EditPrefabContentsScope`)에서 `SerializedObject`로 `_iconImage`/`_nameText`/`_descriptionText`/`_typeText`/`_selectButton`/`_canvasGroup` 필드에 연결하는 2단계 구조다. 동일한 패턴으로 `DamageText`라는 이름의 `GameObject`를 생성 블록에 추가하고, 참조 연결 블록에 `TextMeshProUGUI damageTmp = root.transform.Find("DamageText")?.GetComponent<TextMeshProUGUI>();`와 `so.FindProperty("_damageText").objectReferenceValue = damageTmp;`를 추가한다. 프리팹이 이미 존재하는 경우에도 "Always connect internal refs" 블록은 항상 실행되므로, 기존 프리팹에도 `DamageText` 자식이 없다면 생성 블록 쪽에 존재 여부 체크 후 추가하는 보정 로직이 필요하다.
...
- 신규 파일: `Assets/_Project/Scripts/UI/SkillSlotIcon.cs`
  ...
  - `x{N}` 배지는 스킬 레벨을 의미하는 것으로 확정됨. `SkillData.CurrentLevel`은 0-based(0, 1, 2)이므로 화면 표시 시 `CurrentLevel + 1`로 보정해 `x1`/`x2`/`x3`로 표시한다.
...
- 신규 파일: `Assets/_Project/Scripts/UI/SkillSlotGroup.cs`
  ...
  - 메서드:
    ...
    ```
    (Active/Passive용으로 별도 메서드를 두되 내부 로직은 동일 패턴이며, 리스트 타입만 다름. 두 메서드 중 실제로 어느 그룹 인스턴스가 어느 메서드를 쓰는지는 `SkillSelectionPanel`이 결정. `SetFilled` 호출 시 `CurrentLevel`에 +1을 더해 1-based 레벨(`x1`/`x2`/`x3`)로 보정하는 부분은 Active/Passive 공통으로 확정됨 — Passive 스킬도 PDF 스펙(`PurpleCow_클라이언트_채용과제.pdf`)상 Lv.1/2/3 레벨별 수치가 존재하고 `SkillData._levels` 배열 구조를 Active와 동일하게 공유하므로, `UpdatePassiveSlots()`도 Active와 동일한 레벨 배지 표시 방식을 그대로 사용한다.)
...
- Editor 스크립트 자동화: `UISetupEditor.cs`의 `Step11_SetupSkillSelectionPanelContent()`에 슬롯 UI 생성/연결 로직을 추가한다. 현재 이 메서드는 `EnsureChildObject` + `PrefabUtility.InstantiatePrefab(skillCardPrefab, ...)`로 `SkillCard` 인스턴스를 필요한 개수만큼 채운 뒤 `SerializedObject`의 `SerializedProperty`(배열 프로퍼티 `_skillCards`, `_allSkillDatas`)에 연결하는 패턴을 쓰고 있으므로, 동일한 방식으로 다음을 추가한다.
  1. `SkillSlotIcon`용 신규 프리팹(`Assets/_Project/Prefabs/UI/SkillSlot.prefab`)을 `Step6_CreateSkillCardPrefab()`과 같은 패턴(존재하지 않으면 `Image`/TMP_Text 자식 생성 후 `SaveAsPrefabAsset`, 이어서 `EditPrefabContentsScope`로 `_iconImage`/`_levelText`/`_filledRoot`/`_emptyRoot` 연결)으로 생성하는 `Step12_CreateSkillSlotPrefab()`을 신설한다.
  2. `SkillSelectionPanel` 오브젝트 하위에 "Active Skill" 라벨(TMP_Text) + `SkillSlotGroup`(Active 4칸), "Passive Skill" 라벨(TMP_Text) + `SkillSlotGroup`(Passive 2칸)을 `EnsureChildObject`로 생성하고, `SkillSlot.prefab`을 `PrefabUtility.InstantiatePrefab()`으로 각 그룹에 필요한 개수(4개/2개)만큼 인스턴스화한 뒤 `SkillSlotGroup._slots` 배열 프로퍼티에 연결한다.
  3. `skillPanel`의 `SerializedObject`에 `_activeSlotGroup`/`_passiveSlotGroup` 필드를 연결한다.
  이 로직을 `Step11_SetupSkillSelectionPanelContent()` 내부(또는 별도 `Step13_SetupSkillSlotGroups()`로 분리 후 `SetupUI()`의 실행 순서에 추가)에 반영하면 `[MenuItem("PurpleCow/Setup/UI Setup")]` 실행만으로 슬롯 UI가 자동 배치/연결된다.
...
- 대상 파일 2: `Assets/_Project/Scripts/UI/HUDPanel.cs`
  ...
  - Editor 스크립트 자동화: `UISetupEditor.cs`의 `Step9_SetupHUDPanelContent()`에 진행률 텍스트 생성/연결 로직을 추가한다. 현재 이 메서드는 `EnsureChildObject(hudPanelObj.transform, "WaveText")`, `"ScoreText"` 등으로 TMP_Text 자식을 생성하고 `SerializedObject`로 `_waveText`/`_scoreText` 필드에 연결하는 패턴을 쓰고 있으므로, 동일하게 `EnsureChildObject(hudPanelObj.transform, "ProgressText")`(또는 `TopBar` 하위 구조가 별도 존재할 경우 그 하위)로 오브젝트를 만들고 `TextMeshProUGUI` 컴포넌트를 추가한 뒤 `so.FindProperty("_progressText").objectReferenceValue = progressTextObj.GetComponent<TextMeshProUGUI>();`를 추가한다. `[MenuItem("PurpleCow/Setup/UI Setup")]` 실행 시 자동으로 배치/연결되므로 수동 Inspector 작업은 불필요하다.
...
- `Assets/_Project/Scripts/Editor/UISetupEditor.cs` — `Step2_SetupHUDCanvas()`에 HP 숫자 텍스트 생성/연결, `Step6_CreateSkillCardPrefab()`에 데미지 텍스트 생성/연결, `Step9_SetupHUDPanelContent()`에 진행률(%) 텍스트 생성/연결 추가, 신규 `Step12_CreateSkillSlotPrefab()`(및 필요 시 `Step13_SetupSkillSlotGroups()`) 추가로 `SkillSlot.prefab` 생성과 `SkillSelectionPanel`의 Active/Passive 슬롯 그룹 배치·연결까지 자동화
...
- `Assets/_Project/Prefabs/UI/SkillSlot.prefab` — `UISetupEditor.cs`의 신규 `Step12_CreateSkillSlotPrefab()`이 자동 생성하는 슬롯 1칸 프리팹

**Editor 스크립트로 자동화 처리 (`UISetupEditor.cs` 수정 대상, `[MenuItem("PurpleCow/Setup/UI Setup")]` 실행으로 반영)**
- `CharacterHpBar`가 붙은 `CharacterHP` 오브젝트에 TMP_Text 자식 추가 및 `_hpText` 필드 연결 — `Step2_SetupHUDCanvas()`에서 처리
- `SkillCardUI` 프리팹에 데미지 텍스트 오브젝트 추가 및 `_damageText` 필드 연결 — `Step6_CreateSkillCardPrefab()`에서 처리
- `SkillSelectionPanel` 프리팹에 "Active Skill"/"Passive Skill" 라벨 + 슬롯 오브젝트(4+2) 배치, `SkillSlotIcon`/`SkillSlotGroup` 프리팹 구성 및 필드 연결 — 신규 `Step12_CreateSkillSlotPrefab()` + `Step11_SetupSkillSelectionPanelContent()`(또는 신규 `Step13`)에서 처리
- `HUDPanel`의 `TopBar` 오브젝트에 진행률(%) 텍스트 배치 및 `_progressText` 필드 연결 — `Step9_SetupHUDPanelContent()`에서 처리

---

## 주의사항

1. **`x{N}` 배지 값의 의미: 확정됨 — 레벨을 의미함**: 슬롯의 `x{N}` 배지는 스킬 레벨을 의미하는 것으로 확정되었다. `SkillData.CurrentLevel`이 0-based(0, 1, 2)이므로 화면 표시 시 `CurrentLevel + 1`로 보정해 `x1`/`x2`/`x3`로 표시한다. `SkillSlotGroup.cs`의 `SetFilled()` 호출부(`UpdateActiveSlots()`/`UpdatePassiveSlots()`)에서 `skills[i].SkillData.CurrentLevel + 1`을 전달하도록 반영되어 있다.
...
3. **Passive 슬롯도 `x{N}` 배지를 그대로 사용: 확정됨**: PDF(`PurpleCow_클라이언트_채용과제.pdf`)의 패시브 스킬 표에도 레벨별(Lv.1/2/3) 수치가 존재함을 확인했고, 이는 `SkillData._levels` 배열이 액티브/패시브 공통 구조인 현재 코드와도 일치한다. 따라서 `SkillSlotGroup.UpdatePassiveSlots()`도 `UpdateActiveSlots()`와 동일하게 레벨 배지(`x{CurrentLevel + 1}`)를 표시하는 것으로 확정한다.
...
5. **컴포넌트 스크립트 작성만으로는 화면에 보이지 않음 — `UISetupEditor.cs` 실행까지 완료해야 함**: 이 계획은 컴포넌트 스크립트(C#) 변경과 함께 `Assets/_Project/Scripts/Editor/UISetupEditor.cs`에 각 항목의 오브젝트 생성/필드 연결 로직을 추가하는 것까지 포함한다. 다만 `UISetupEditor.cs` 자체를 수정하는 것만으로는 씬/프리팹에 실제 반영되지 않으며, Unity 에디터에서 `[MenuItem("PurpleCow/Setup/UI Setup")]`(메뉴 경로: `PurpleCow > Setup > UI Setup`)을 실제로 실행해야 HP 숫자 텍스트, 데미지 텍스트, 슬롯 UI, 진행률(%) 텍스트가 씬/프리팹에 생성되고 필드가 연결된 상태로 화면에 보인다. 따라서 구현 완료 후 반드시 해당 메뉴 아이템을 실행하는 단계까지 완료해야 하며, 코드 리뷰만으로는 작업이 끝난 것이 아니다.
