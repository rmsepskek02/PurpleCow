...
이 문서는 볼 궤도(트레젝토리) 프리뷰와 관련해 사용자가 지적한 네 가지 문제, 즉 (1) 궤적이 터치할 때만 보이는 문제, (2) 터치 조준 시 손가락 방향과 궤적 각도가 미묘하게 어긋나는 정확성 문제, (3) 궤적 프리뷰의 레드닷/링/점선 색상과 크기가 원본 게임과 다른 문제, (4) 이슈 2 수정 이후 실제 플레이 테스트 과정에서 추가로 발견된, 터치 시작 순간과 궤적 사이의 괴리감(조준 모델 자체의 문제)을 다룬다. 이슈 1~3은 이미 구현이 완료되어 main에 반영된 상태이며, 이슈 4는 이슈 2 구현 완료분에 대한 실제 플레이 테스트 이후 사용자가 재발견하여 이번에 추가로 반영하는 내용이다. 현재 구현(`InputHandler`, `TrajectoryPreview`, `BallLauncher`)을 코드 레벨에서 조사하고, 각 문제의 원인을 분석한 뒤 다음 단계(plan.md)에서 다룰 방향을 정리한다. 구체적인 코드 수정은 이 문서에서 다루지 않는다.
