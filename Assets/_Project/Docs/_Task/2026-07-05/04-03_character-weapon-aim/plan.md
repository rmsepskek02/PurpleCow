...
- 이 좌표들은 `Character_Main.png`(합본, 141×178 rect) 안에서 각 파츠가 차지하는 위치를 이미지 템플릿 매칭으로 역산한 추정치이며, 레퍼런스 합본 이미지 자체가 "방울 왼쪽·무기 오른쪽"인 오른쪽 조준 기본 자세였으므로 그 자세의 배치값으로는 그대로 유효하다. 특히 Weapon/WeaponPivot 쪽은 다른 파츠보다 매칭 신뢰도가 낮았다. Unity 씬에 `Character_Main.png`를 반투명 참고 오버레이(예: 임시 SpriteRenderer, alpha 0.3~0.5)로 띄워놓고, Head/Body/WeaponPivot을 눈으로 겹쳐보며 최종 위치를 미세조정한다. 오버레이 오브젝트는 검증 후 삭제하거나 비활성화한다.
...
- 좌우 반전 자세(방울 오른쪽·무기 왼쪽, 조준 각도 음수 구간)는 원본 게임 레퍼런스 스크린샷 10장 중 단 한 장에서도 확인되지 않았다(research.md의 "추가 조사" 섹션 참고). 사용자의 구두 설명에 근거해 오른쪽 자세를 좌우 반전한 형태로 대칭 적용한다는 전제로 진행하므로, 구현 후 반드시 Unity에서 왼쪽 조준을 시도해보며 방울/무기 위치와 회전 방향이 자연스러운지 육안 검증이 필요하다.
