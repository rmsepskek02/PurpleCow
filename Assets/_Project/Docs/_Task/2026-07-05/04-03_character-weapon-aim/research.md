...
## 추가 조사 — 레퍼런스 스크린샷 재크롭 분석 (1차 구현 이후 재확인)

이미 위 결론을 바탕으로 plan.md 1차 버전이 작성되었고, dev 에이전트가 weapon meta rect 수정 · `CharacterAimController.cs` · `CharacterSetupEditor.cs` · `SceneSetupEditor.cs` Step11까지 1차 구현을 완료해 커밋한 상태였다. 그 이후 오케스트레이터가 사용자와 추가로 논의하면서, `Assets/_Project/Docs/targetUI/`의 원본 게임 레퍼런스 스크린샷 중 캐릭터가 보이는 6장을 Python(PIL)으로 캐릭터 부분만 크롭(이미지 세로 62~90% 구간)해서 2배 확대한 뒤 다시 자세히 비교 분석했다.

6장 중 실제 게임플레이 장면인 4장(`KakaoTalk_20260703_115951058_01.jpg`, `_03.jpg`, `KakaoTalk_20260701_190324151.jpg`, `_02.jpg`)은 캐릭터가 조준 방향에 따라 무기를 들고 있는 모습이 선명하게 보여 분석에 사용했다. 나머지 4장(`KakaoTalk_20260701_190324151_03.jpg`, `_04.jpg`, `KakaoTalk_20260703_115951058.jpg`, `_02.jpg`)은 스킬 선택 UI 화면이라 캐릭터가 배경에 흐리게만 보여 참고할 수 없었다.

**확인된 사실:**

- 게임플레이 장면 4장 모두 조준 각도가 0~90도(정면 위쪽~오른쪽 수평) 범위였다. 4장 전부 공통적으로 방울 장식(캐릭터 후드에 달린 주황색 방울)은 화면 기준 캐릭터 왼쪽에, 무기는 오른쪽에 위치하는 자세를 유지했다.
- 조준 각도가 0도(정면 위쪽)에 가까울수록 무기가 머리 위쪽 가까이 거의 수직으로 세워져 있고, 90도(수평)에 가까울수록(사막 스테이지 스크린샷에서 확인) 무기가 어깨 옆에서 거의 수평으로 뻗어 있어, 무기 각도가 조준 각도를 그대로 따라가는 것으로 확인된다.
- 무기의 손잡이 쪽(캐릭터가 쥔 지점)은 각도가 바뀌어도 대략 어깨 근처의 비슷한 자리에 머물러 있고, 갈고리 끝(먼 쪽)만 크게 호를 그리며 움직인다 — 즉 회전축이 무기 자체의 내부 지점이 아니라 캐릭터 몸 쪽에 가까운 고정점이라는 것이 시각적으로 확인된다.
- 기존에 Head/Weapon 배치 좌표 산출에 썼던 `Character_Main.png` 합본 자체가 정확히 이 "방울 왼쪽·무기 오른쪽"(오른쪽 조준) 자세였다는 것도 재확인했다.
- **왼쪽 조준(음수 각도, 270~360도 구간)에 해당하는 예시는 10장의 레퍼런스 스크린샷 중 단 한 장도 없었다.** 즉 좌우 반전 자세(방울 오른쪽·무기 왼쪽)는 이미지로 직접 검증하지 못했으며, 사용자의 구두 설명("반대 상황도 마찬가지야")에 근거해 오른쪽 자세를 좌우 반전한 형태로 대칭 적용한다는 전제로 진행한다. 이 전제는 plan.md의 좌우 반전 설계에서도 다시 한 번 명시한다.
- 사용자가 직접 원본 게임을 플레이한 경험을 근거로, 머리는 무기보다 훨씬 작은 범위(0~10도)로만 회전한다고 확정했다. 정지 이미지만으로는 이 미세한 머리 회전 여부를 육안으로 검증하기 어려웠다.

이 재분석 결과로, 기존 plan.md의 핵심 설계 중 (1) 18도 각도 오프셋 보정, (2) 무기 자신의 pivot을 회전축으로 삼는 방식, (3) 좌우 반전 없음, (4) 머리 회전 없음의 네 가지가 실제 원본 게임 동작과 다르다는 것이 확인되어, plan.md의 무기 회전 관련 설계를 전면 수정했다.

### 왼쪽 조준 레퍼런스 추가 확인 및 좌표 부호 오류 정정

위 재분석 이후, 사용자가 왼쪽 조준(음수 각도) 중인 원본 게임 스크린샷 3장을 새로 제공했다. 오케스트레이터가 이 3장을 기존에 분석한 오른쪽 조준 스크린샷, 그리고 `Character_Main.png` 합본 이미지와 다시 나란히 비교한 결과 다음을 확인했다.

- 새로 제공된 왼쪽 조준 스크린샷 3장은 방울 장식이 캐릭터 화면 기준 오른쪽, 무기가 왼쪽에 위치하는 자세였다. 이는 오른쪽 조준 자세(방울 왼쪽·무기 오른쪽)를 정확히 좌우 반전한 모습으로, 위 섹션에서 "10장 중 단 한 장도 확인되지 않았다"고 남겨두었던 왼쪽 조준 예시가 이제 확보되었다. 즉 좌우 반전 가정이 사용자의 구두 설명이 아니라 이미지로 직접 검증되었다.
- **중요한 정정**: `Character_Main.png` 합본 이미지를 다시 확인한 결과, 이 합본은 방울이 오른쪽·무기가 왼쪽인 자세 — 즉 "왼쪽 조준" 자세였다. 위 섹션에서 "합본 자체가 오른쪽 조준(방울 왼쪽·무기 오른쪽) 자세였다"고 재확인했던 내용은 착오였고, 실제로는 그 반대다.
- 이 착오로 인해 1차·2차 구현 모두 합본 이미지의 파츠 좌표를 "오른쪽 조준 기본 자세(양수 각도, 좌우 반전 없음)"의 좌표로 잘못 취급해서, plan.md 4단계에 기록된 Head/WeaponPivot 로컬 X좌표가 실제와 반대 부호로 산출되어 있었다(Head를 오른쪽 `+0.34`, WeaponPivot을 왼쪽 `-0.29`로 배치). 합본이 왼쪽 조준 자세라면 방울(머리 쪽 장식)은 오른쪽, 무기는 왼쪽에 있어야 하므로, 오른쪽 조준 기본 자세를 만들려면 Head는 왼쪽(`-0.34`), WeaponPivot은 오른쪽(`+0.29`)에 배치해야 부호가 맞는다.
- 이 오류는 오케스트레이터가 dev 에이전트에게 별도로 지시해 `CharacterSetupEditor.cs`의 Head/WeaponPivot X좌표 부호를 수정하는 것으로 바로잡았다(Head `(-0.34, 0.58)`, WeaponPivot `(0.29, 0.65)`로 정정). 이 문서와 plan.md의 좌표 기록도 함께 정정한다.
