...
## 재검토 — 무기 길이 기반 발사 위치 가정 오류 발견

이 섹션은 위 "결론"에서 정리했던 발사 시작점 공식(`Character 위치 + LaunchDirection(정규화) × 무기 길이(0.6612)`)이 잘못된 전제에 기반했음이 이후 재검토로 드러나, 이를 바로잡기 위해 추가되었다. 위 "현재 상태"/"관련 파일 및 의존성"/"결론" 내용은 재검토 이전 시점의 조사 기록으로 그대로 남겨두고, 아래 내용이 최종 결론을 대체한다.

### 재검토 경위

- 오케스트레이터가 처음에 실제 게임 스크린샷 극소수(2~3장)만 보고 "무기 끝에서 볼이 발사된다"고 추측해 위 공식을 만들었다.
- 이후 사용자 지적으로 `Assets/_Project/Docs/targetUI/` 폴더의 실제 게임 레퍼런스 스크린샷 전체(총 12장, 이 중 캐릭터가 조준 중인 장면 6장 정도)를 다시 확인했다.
- 여러 장을 비교한 결과, 무기(지팡이) 스프라이트가 가리키는 방향과 실제 볼 궤적(점선으로 표시되는 발사 궤도) 방향이 서로 다른 경우를 발견했다(예: 궤적은 캐릭터 기준 우상단으로 일직선으로 나가는데, 무기 그림은 좌상단을 향하고 있는 스크린샷이 확인됨).
- 이를 근거로 결론: **무기 스프라이트는 대략적인 조준 방향을 보여주는 장식적 요소일 뿐이며, 실제 볼은 항상 캐릭터의 고정된 중심 위치에서 발사된다.** 무기 길이만큼 캐릭터로부터 떨어진 지점에서 발사된다는 기존 가정은 틀렸다.

### 확정된 새 설계 (수정 사항)

- **`LaunchOrigin`(발사 시작점) = `Character` 위치 그대로** (무기 길이 오프셋 제거). 즉 `BallLauncher.LaunchOrigin`은 더 이상 `LaunchDirection`이나 무기 길이를 계산에 사용하지 않고, `CharacterAimController.Instance.transform.position`만 반환하면 된다 — 사실상 이미 앞선 컴파일 오류 수정 때 추가한 `BallLauncher.CharacterPosition` 프로퍼티와 동일한 값이 된다.
...
## 재검토 3 — flipX 부호 반전 버그 발견

이 섹션은 위 "재검토 2"와는 별개로, `CharacterAimController.cs`에서 Body/Head/Weapon 파츠의 좌우 반전(`flipX`) 값을 결정하는 부호 자체가 반대로 걸려 있던 버그를 새로 발견해 정리한 것이다.

### 근거

실제 게임(통통 디펜스: 핀볼 마스터) 참고 스크린샷 두 장을 픽셀 단위로 크롭 분석했다.

- `Assets/_Project/Docs/targetUI/KakaoTalk_20260704_181516769_01.jpg` — 조준이 거의 위쪽(살짝 왼쪽으로 기움)인 경우. 무기(스태프)가 캐릭터 왼쪽 어깨에, 후드 방울 장식이 오른쪽에 위치.
- `Assets/_Project/Docs/targetUI/KakaoTalk_20260701_190324151_01.jpg` — 조준이 오른쪽인 경우. 무기가 오른쪽 어깨로, 방울 장식이 왼쪽으로 이동(완전히 좌우 미러링된 모습).

즉 프로젝트의 기본 스프라이트(`Character_Main.png`, `flipX = false` 상태)는 "조준 왼쪽/위" 포즈를 나타내고 있으며, 조준이 오른쪽으로 갈 때 `flipX = true`로 미러링되어야 한다.
...
`_facingRight`가 `true`(오른쪽 조준)일 때 `!_facingRight` = `false`가 되어 오히려 반전이 안 걸리는, 정확히 반대인 로직이었다.

회전 각도 공식(`aimAngle`)은 벡터 대수로 재검증한 결과 문제가 없었다(미러링된 케이스에도 동일 공식이 올바르게 좌우대칭을 만들어냄을 좌표 계산으로 확인). 유일한 버그는 `flipX` 부호였다.

### 확정된 수정 사항

- `CharacterAimController.cs`의 반전 판정을 `!_facingRight` → `_facingRight`로 단순화한다(Body/Head/Weapon 모두 동일 값 사용, 기존과 동일하게 세 파츠에 같은 값을 대입).

---

## 재검토 4 — 무기 회전 피벗이 캐릭터 중심이 아닌 스태프 자체 중심이었던 문제

이 섹션은 사용자가 "스태프의 회전이 스태프 중심에서 회전하던데 스태프의 회전은 캐릭터 중심을 기준으로 회전해야해"라고 지적한 내용을 조사해 정리한 것이다.

### 원인

`Character_main_weapon.png.meta`의 스프라이트 피벗이 `{x: 0.39, y: 0.43}`으로, 스태프 그래픽의 중간(밑동이 아닌 중간 지점)에 위치해 있었다. Weapon GameObject는 Character의 local `(0, 0, 0)`에 배치되어 있어 캐릭터 원점과 일치하지만, 실제 회전은 Transform 위치가 아니라 스프라이트의 피벗 지점을 기준으로 일어나기 때문에, 피벗이 스태프 중간에 있으면 회전 시 스태프가 캐릭터 중심이 아니라 자기 자신의 중간을 축으로 도는 것처럼 보인다.

참고로 "재검토" 이전 시점("현재 상태" 섹션 47~55행)에서는 이 피벗 값(`{0.39, 0.43}`)을 "그립 위치로 이미 조정 완료된 상태"라고 조사했었으나, 실제로는 그립(밑동)이 아니라 스태프 그래픽의 중간 지점에 가까웠던 것으로 이번에 재확인되었다.

### 분석

무기 스프라이트(64x116px)를 픽셀 단위로 스캔해서 스태프 밑동(손잡이 쪽) 좌표를 측정했다. 밑동 근처 좌표는 대략 `{x: 0.09, y: 0.12}`(정규화 좌표, Unity 좌하단 원점 기준)로 나왔다.

### 확정된 수정 사항

- 피벗을 `{x: 0.09, y: 0.12}`로 이동한다.
- 피벗 변경 시 rest-pose(회전 0도) 렌더링 위치가 달라지므로, 기존 시각적 정렬을 유지하기 위해 Weapon의 local position을 보정해야 한다. 계산식: `보정 position = spriteSize(unit) × (신규피벗 - 기존피벗)`, `spriteSize = (0.64, 1.16)`(`spritePixelsToUnits = 100` 기준), 결과 ≈ `(-0.192, -0.36, 0)`.
- `CharacterLaunchOrbitSetupEditor.cs`의 Weapon 생성 시 local position을 `Vector3.zero`에서 `(-0.192, -0.36, 0)`으로 변경한다.

### 주의

이 두 수치(피벗 `{0.09, 0.12}`, 보정 position `(-0.192, -0.36, 0)`)는 픽셀 분석 기반 추정치이며, 실제 Unity Editor에서 사용자가 시각적으로 확인 후 미세 조정이 필요할 수 있다. (참고: 이후 plan.md "수정 4"의 "정정" 섹션에서 스프라이트 폭 계산 오류가 바로잡혀 보정 position 값이 `(-0.177, -0.36, 0)`으로 대체되었다. 이 문서는 재검토 4 작성 당시 기록을 그대로 남겨둔다.)

---

## 재검토 5 — flipX 부호 수정 반영 후 추가로 발견된 두 가지 문제

이 섹션은 위 "재검토 3"에서 확정된 `flipX` 부호 수정(`!_facingRight` → `_facingRight`)이 실제로 코드에 반영된 이후, 그 과정에서 추가로 발견된 두 가지 별개의 문제를 정리한 것이다.

### 1. `_facingRight` 초기값이 바뀐 부호와 짝을 이루지 못하는 문제

`flipX` 판정 공식의 부호가 `!_facingRight`에서 `_facingRight`로 바뀌었으므로, `_facingRight`의 초기값도 이 새 부호에 맞게 재검토가 필요하다는 점이 확인되었다. `CharacterAimController`의 `_facingRight` 필드가 여전히 `true`로 초기화되어 있으면, 게임 시작 직후 아직 조준 입력이 들어오기 전(첫 프레임)에는 `Update()`의 반전 판정 로직(데드존을 넘는 조준 입력이 있을 때만 `_facingRight`를 갱신하는 조건문, 재검토 3의 "확정된 수정 사항" 참고)이 아직 `_facingRight`를 한 번도 갱신하지 않은 상태이므로, 초기값 `true`가 그대로 `flipX = true`(반전됨)로 적용된다. 그런데 재검토 3에서 확인한 대로 프로젝트의 기본 스프라이트(`flipX = false`)는 "조준 왼쪽/위" 포즈를 나타내므로, `_facingRight`의 기본(미조준) 상태는 "왼쪽을 보고 있는" 것으로 간주되어야 하고, 이는 `_facingRight = false`에 대응한다. 즉 부호를 고친 뒤에는 초기값도 `false`로 맞춰야 게임 시작 직후 첫 프레임에 캐릭터가 뒤집힌 포즈로 잠깐 보이는 현상을 막을 수 있다.

### 2. 무기(Weapon)가 조준 방향에 따라 반대쪽 어깨로도 위치를 옮겨야 하는 문제

사용자가 실제 게임 참고 이미지를 다시 확인해 다음을 재확인했다: 왼쪽 조준 시 무기가 캐릭터의 왼쪽 어깨 쪽에 위치하고, 오른쪽 조준 시 무기가 오른쪽 어깨 쪽으로 옮겨간다. 즉 무기는 단순히 `flipX`(비트맵 반전)와 회전만으로는 이 시각적 결과를 만들어낼 수 없고, 실제 부착 지점(로컬 X 좌표) 자체가 좌우로 이동해야 한다.

기존 설계(재검토 4에서 확정된 값 포함)에서는 Weapon의 `localPosition`이 항상 고정된 값이었기 때문에, `flipX`가 걸려도 부착 지점 자체는 항상 캐릭터 중심 기준 왼쪽에 머물러 있었다. 이는 "재검토 2"에서 확정했던 "Head/Body/Weapon 모두 반전 시 위치를 이동시키지 않고 `flipX`만 사용한다"는 원칙과 정면으로 배치되는 사례가 새로 발견된 것이다.

사용자와의 논의 결과, 이 문제는 Body/Head에는 해당하지 않고 **Weapon에만 해당하는 예외**로 확인되었다. Body/Head는 실제 원본 게임에서도 반전 시 위치가 이동하지 않는 것으로 재확인 완료된 상태(재검토 2, 3의 확정 사항 그대로 유지)이며, 무기(스태프)만 조준 방향에 따라 캐릭터의 왼쪽 어깨 또는 오른쪽 어깨로 부착 지점 자체가 이동하는 것이 원본 게임의 실제 동작임이 확인되었다. 즉 Weapon은 `_facingRight` 상태에 따라 로컬 X 좌표의 부호가 반전되어야 하고(Y좌표는 고정), Body/Head는 기존대로 위치 불변(`flipX`만 적용)이 유지되어야 한다.

이 문제는 "재검토 2"에서 다뤘던 문제(Head/Body/Weapon 세 파츠 모두에 동일하게 걸려 있던, 반전 시 위치를 이동시키는 잘못된 로직을 제거한 것)와는 반대 방향의 예외 사례로, 세 파츠를 다시 완전히 동일하게 취급할 수는 없고 Weapon만 별도로 위치 반전 로직을 다시 추가해야 한다는 점에서 구분된다.
