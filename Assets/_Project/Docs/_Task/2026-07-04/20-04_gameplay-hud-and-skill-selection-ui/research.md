...
5. **~~WaveBar 신규 구현~~ → CharacterXpBar 상단 재배치 + TopBar 신규 계산 로직 (정정됨, 위 "추가 정정" 참고)**: 별도의 WaveBar는 존재하지 않는 것으로 확인되었다. 상단 두 번째 바는 기존 `CharacterXpBar.cs`를 그대로 재사용하되 화면 배치 위치만 상단(TopBar 바로 아래)으로 옮긴다. 대신 TopBar의 % 진행바가 "스테이지 전체 누적 처치율"을 의미하도록 `WaveManager.cs`에 신규 계산 로직(스테이지 전체 몬스터 총수 사전 계산)과 신규 이벤트(몬스터 사망마다 발행)를 추가해야 한다.
6. **문서 통합 범위**: 이번 task는 HUD 진행도 개편과 삼택지 카드 개편(New! 라벨 추가 포함) 두 가지를 하나의 research.md/plan.md로 통합하여 다룬다.

---

## 추가 정정 (사용자 실플레이 확인 반영)

최초 조사 시점에는 원본 게임 상단의 두 진행바를 다음과 같이 추측했었다: 빨간 바(TopBar %)=현재 웨이브 내 처치율, 금색 바=전체 20웨이브 중 진행도(웨이브 번호 배지). 이후 사용자가 원본 게임 실제 플레이 경험을 근거로 다음과 같이 정정해줬다.

- 빨간 바(TopBar %) = **스테이지 전체 누적 처치율**(현재 웨이브 내 기준이 아님, 웨이브 전환 시 리셋되지 않음)
- 금색 바 = 오케스트레이터가 "WaveBar"라고 불렀던 것은 실제로는 존재하지 않으며, 이 금색 바는 **캐릭터 경험치(XP)바**다. 이미 구현되어 있는 `CharacterXpBar.cs`/`CharacterManager`(`OnXpChanged`/`OnLevelUp` 이벤트) 시스템과 완전히 동일하며, 경험치가 차서 레벨업하면 삼택지 스킬 선택 UI가 열리는 바로 그 시스템이다. 새로 구현할 필요 없이 화면상 위치만 하단(기존 UIRules.md 설계)에서 상단(TopBar 바로 아래)으로 재배치하면 된다(사용자가 재배치 쪽으로 명시적으로 결정)
- `WaveManager.cs`에 누적 처치 수(`_totalKillCount`)는 이미 있으나, "스테이지 전체 몬스터 총수"(분모)를 계산하는 로직은 없음을 재확인함 — 현재 스폰 공식(`BaseSpawnCount + SpawnCountPerWave × 웨이브인덱스`, 그리드 용량 상한 적용)을 웨이브 0~19까지 미리 합산하는 신규 계산이 필요
- 몬스터가 죽을 때마다 즉시 갱신되는 이벤트가 없어(`OnMonsterCountChanged`는 웨이브 내 기준, `OnKillCountReached`는 5킬마다만 발행) 신규 이벤트가 필요함이 확인됨

이 정정은 plan.md에 반영되었다.
