...
- **기존 겹침 보정**: 이미 소량 겹친 경우에는 음수 간격만큼 뒤쪽 몬스터를 위로 보정한다.
...
- **정적 검증**: 간격 계산 6개 사례와 런타임/Editor C# 빌드 오류 0개.
- **남은 검증**: Unity 플레이 모드에서 직접 대상만 빙결되는지, 먼 후방은 계속 이동하는지, 접촉 시에만 정지하는지, 가로 2칸 포함 모든 크기 조합이 겹치지 않는지 확인한다.
